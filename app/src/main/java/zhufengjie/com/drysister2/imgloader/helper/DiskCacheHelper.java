package zhufengjie.com.drysister2.imgloader.helper;
...
public class DiskCacheHelper {
    ...
    public DiskCacheHelper(Context mContext) {
        ...
        if (getUsableSpace(diskCacheDir) > DISK_CACHE_SIZE) {//如果可用空间大于将设置的磁盘缓存区大小
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir, 1, 1, DISK_CACHE_SIZE);//创建缓存区
